...
doInstall:
  ...
  ; Whenever this driver is updated in the repo QMLCURRENTDRIVERVERSION must be bumped by 1
  File ${DRIVER_MSI}
  ...
  WriteRegExpandStr HKLM "SOFTWARE\Microsoft\Windows\Windows Error Reporting\LocalDumps\${EXENAME}.exe" "DumpFolder" "%LOCALAPPDATA%\QMLCrashDumps"

  ; QML stores its own driver version key to prevent installation if already up to date
  ; This prevents running the driver install a second time which will start up in repair mode which is confusing
  !define QMLDRIVERVERSIONKEY "SOFTWARE\QGroundControlUAVDrivers"
  !define QMLCURRENTDRIVERVERSION 3
...
driversInstalled:
  ...
  ReadRegDWORD $0 HKCU "${QMLDRIVERVERSIONKEY}" "version"
  IntCmp $0 ${QMLCURRENTDRIVERVERSION} done driversOutOfDate done
...
installDrivers:
  ...
  WriteRegDWORD HKCU "${QMLDRIVERVERSIONKEY}" "version" ${QMLCURRENTDRIVERVERSION}
  goto done
