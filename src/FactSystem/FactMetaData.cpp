...
const char* FactMetaData::qmlFileType                           = "FactMetaData";
const char* FactMetaData::_jsonMetaDataDefinesName              = "QML.MetaData.Defines";
const char* FactMetaData::_jsonMetaDataFactsName                = "QML.MetaData.Facts";
...
const char* FactMetaData::_qmlRebootRequiredJsonKey =   "qmlRebootRequired";
...
const FactMetaData& FactMetaData::operator=(const FactMetaData& other)
{
    ...
    _qmlRebootRequired      = other._qmlRebootRequired;
...
void FactMetaData::_setAppSettingsTranslators(void)
{
    ...
    if (!_enumStrings.count() && (type() == valueTypeDouble || type() == valueTypeFloat)) {
        for (size_t i=0; i<sizeof(_rgAppSettingsTranslations)/sizeof(_rgAppSettingsTranslations[0]); i++) {
            ...
            UnitsSettings* settings = qmlApp()->toolbox()->settingsManager()->unitsSettings();
...
const FactMetaData::AppSettingsTranslation_s* FactMetaData::_findAppSettingsUnitsTranslation(const QString& rawUnits, UnitTypes type)
{
    for (size_t i=0; i<sizeof(_rgAppSettingsTranslations)/sizeof(_rgAppSettingsTranslations[0]); i++) {
        ...
        uint unitOption = 0;
        auto unitsSettings = qmlApp()->toolbox()->settingsManager()->unitsSettings();
        switch (type) {
        case UnitHorizontalDistance:
...
FactMetaData* FactMetaData::createFromJsonObject(const QJsonObject& json, QMap<QString, QString>& defineMap, QObject* metaDataParent)
{
    ...
    QList<JsonHelper::KeyValidateInfo> keyInfoList = {
        ...
        { _qmlRebootRequiredJsonKey,    QJsonValue::Bool,   false },
    ...
    bool qmlRebootRequired = false;
    if (json.contains(_qmlRebootRequiredJsonKey)) {
        qmlRebootRequired = json[_qmlRebootRequiredJsonKey].toBool();
    }
    metaData->setqmlRebootRequired(qmlRebootRequired);
...
QMap<QString, FactMetaData*> FactMetaData::createMapFromJsonFile(const QString& jsonFilename, QObject* metaDataParent)
{
    ...
    QString errorString;
    int version;
    QJsonObject jsonObject = JsonHelper::openInternalQMLJsonFile(jsonFilename, qmlFileType, 1, 1, version, errorString);
    if (!errorString.isEmpty()) {
